    ...
    private void StateMachine(PlayerState state)
    {
        ...
        switch (state)
        {
            ...
            case PlayerState.FALLING:
                ...
                // When you land on the ground changes state
                if (coll.onGround && !groundTouch)
                {
                ...
                // Changes state to WALLSLIDING if touches wall before touching ground
                if (coll.onWall && !coll.onGround)
                {
            ...
            case PlayerState.CLIMBING:
                ...
                if (!coll.onWall || !Input.GetButton("Fire2"))
                {
                    ...
                    // Changes state to grounded when touches ground
                    if (coll.onGround && !groundTouch)
                    {
                        // GroundTouch() resets the dash, as you can only dash once per jump
                        GroundTouch();
            ...
            case PlayerState.WALLSLIDING:
                ...
                if (coll.onGround)
                {
                    // When you touch on the ground changes state
                    if (!groundTouch)
                    {
                        // GroundTouch() resets the dash, as you can only dash once per jump
                        GroundTouch();
            ...
            case PlayerState.JUMPING:
                ...
                if (rb.velocity.y <= 0.0f && !groundTouch)
                {
                    currentState = PlayerState.FALLING;
                }
